...
export default function TayseerLanding() {
  ...
  return (
    <div
      className="min-h-dvh flex flex-col text-[var(--c-text)] bg-[var(--bg)]"
      style={paletteVars}
    >
      {/* ===== HEADER ===== */}
        ...
        {menuOpen && (
          <div className="fixed inset-0 z-50 bg-[var(--c-primary)]/90 backdrop-blur-md flex flex-col items-center justify-center gap-6 text-lg font-medium text-white">
            ...
            <Link
              href="/auth/login"
              onClick={() => setMenuOpen(false)}
              className="rounded-xl bg-white text-[var(--c-primary)] px-6 py-3 shadow font-semibold hover:bg-[var(--c-accent)] transition"
            >
              Log in
            </Link>
          </div>
...
function FeatureBullet({
  ...
  return (
    ...
    >
      ...
      />
      <div>
        <p className="font-medium text-[var(--c-primary)]">{title}</p>
        <p className="text-[var(--c-text)]/80 text-sm mt-1">{children}</p>
      </div>
    </motion.div>
  );
}
